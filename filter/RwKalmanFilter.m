...
% EKF for rw attitude control
    ...
    methods
        ...
        function estimatedX = estimate(this, t0, x0, bModelOrb, bSensor, sunModelOrb, sunSensor, rwCtrl)
            ...
            if ~isempty(this.sat.sunSensors) && ~any(isnan(sunSensor)) 
                Con = true;
            else 
                Con = false;
            end 

            estimatedX = this.correction(predictedX, predictedP, bModelOrb, bSensor, sunModelOrb, sunSensor, Con);
        end
    ...
    methods (Access = protected)
        ...
        % EKF corrector
        function estimatedX = correction(this, predictedX, predictedP, bModelOrb, bSensor, sunModelOrb, sunSensor, Con)
            ...
            H = RwKalmanFilter.calcObservationMatrix(bModelBody / bModelNorm, sunModelBody, Con);
            K = this.calcKalmanGain(predictedP, H, bModelNorm, Con);
...
%% state-dependent EKF matrices
        ...
        function K = calcKalmanGain(this, P, H, bModelNorm, Con)
            if Con == true
                measurementsCov = this.R;
                measurementsCov(1:3, 1:3) = this.R(1:3, 1:3) / bModelNorm^2;
            else
                measurementsCov = this.R(1:3,1:3);
                measurementsCov(1:3, 1:3) = this.R(1:3, 1:3) / bModelNorm^2;
            end 
    ...
    methods (Static)
        ...
        function H = calcObservationMatrix(bModel, sunModel, Con)
            if Con == true
